...
STRUCT_DIR := ./libs/datastructs/
REND_DIR := ./libs/rend/
STRUCT_SOURCE_FILES := $(wildcard $(STRUCT_DIR)*.cpp)
STRUCT_OBJECT_FILES := $(patsubst $(STRUCT_DIR)%.cpp, $(BUILD_DIR)/%.o, $(STRUCT_SOURCE_FILES))
...
# Set up flags
STRUCT_COMPILER_FLAGS := -I$(STRUCT_DIR)
...
STRUCT_LINKER_FLAGS := $(STRUCT_OBJECT_FILES)
...
debug: $(STRUCT_OBJECT_FILES) $(REND_OBJECT_FILES)
	$(COMPILER) $(COMPILER_OPTIONS) $(DEBUG_OPTIONS) -o $(OBJECT_FILE) $(SOURCE_FILE) $(SDL_COMPILER_FLAGS) $(STRUCT_COMPILER_FLAGS) $(REND_COMPILER_FLAGS)
	$(COMPILER) $(LINKER_OPTIONS) $(DEBUG_OPTIONS) -o $(EXECUTABLE) $(OBJECT_FILE) $(REND_LINKER_FLAGS) $(STRUCT_LINKER_FLAGS) $(SDL_LINKER_FLAGS)
	./$(EXECUTABLE)

# Rule to build for high performance executable (for manually testing interaction)
speedy: $(STRUCT_OBJECT_FILES) $(REND_OBJECT_FILES)
	$(COMPILER) $(COMPILER_OPTIONS) $(SPEEDY_OPTIONS) -o $(OBJECT_FILE) $(SOURCE_FILE) $(SDL_COMPILER_FLAGS) $(STRUCT_COMPILER_FLAGS) $(REND_COMPILER_FLAGS)
	$(COMPILER) $(LINKER_OPTIONS) $(SPEEDY_OPTIONS) -o $(EXECUTABLE) $(OBJECT_FILE) $(REND_LINKER_FLAGS) $(STRUCT_LINKER_FLAGS) $(SDL_LINKER_FLAGS)
	./$(EXECUTABLE)

# Rule for building all of the the DisplayWindow classes
$(BUILD_DIR)/%.o: $(STRUCT_DIR)%.cpp
	@mkdir -p $(BUILD_DIR)
	$(COMPILER) $(COMPILER_OPTIONS) -c -o $@ $^ $(SDL_COMPILER_FLAGS) $(STRUCT_COMPILER_FLAGS) $(REND_COMPILER_FLAGS)

# Rule for building all of the the rendering classes
$(BUILD_DIR)/%.o: $(REND_DIR)%.cpp
	@mkdir -p $(BUILD_DIR)
	$(COMPILER) $(COMPILER_OPTIONS) -c -o $@ $^ $(SDL_COMPILER_FLAGS) $(STRUCT_COMPILER_FLAGS)
